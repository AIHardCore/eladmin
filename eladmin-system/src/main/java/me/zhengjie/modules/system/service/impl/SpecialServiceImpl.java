...
public class SpecialServiceImpl implements SpecialService {
    ...
    @Override
    public List<SpecialDto> queryAll() {
        Sort sort = Sort.by(Sort.Direction.ASC, "sort");
        return specialMapper.toDto(specialRepository.findAll(sort));
    }
    ...
    public void download(List<SpecialDto> all, HttpServletResponse response) throws IOException {
        ...
        for (SpecialDto specialIssue : all) {
            Map<String,Object> map = new LinkedHashMap<>();
            map.put("名称", specialIssue.getName());
            map.put("封面", specialIssue.getCover());
            map.put("排序", specialIssue.getSort());
            map.put("状态", specialIssue.getEnabled());
            map.put("创建者", specialIssue.getCreateBy());
            map.put("更新者", specialIssue.getUpdateBy());
            map.put("创建日期", specialIssue.getCreateTime());
            map.put("更新时间", specialIssue.getUpdateTime());
            list.add(map);
        }
